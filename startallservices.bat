start cmd /k startdb.bat
timeout 50 >nul
netstat -an | findstr LISTEN | findstr 3040
if %ERRORLEVEL% == 0 goto :batchservice
echo "Errors encountered while starting db services.  Exited with status: %errorlevel%"
goto :endofscript
...
:enlimobile
echo "starting enli mobile"
start cmd /k startmobile.bat
timeout 100 >nul
netstat -an | findstr LISTEN | findstr 3050
if %ERRORLEVEL% == 0 goto :completed
echo "Errors encountered during execution.  Exited with status: %errorlevel%"
goto :endofscript
